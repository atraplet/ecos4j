...
class BindingsTest {
    ...
    void versionReturnsNonEmptyString() {
        val version = ecos_h.ECOS_ver().getUtf8String(0);
    ...
    void solvePortfolioOptimizationProblemReturnsExpectedSolution() {
        ...
        try (val arena = Arena.ofConfined()) {
            ...
            val qSeg = arena.allocateArray(C_LONG_LONG, new long[]{5});
            val gprSeg = arena.allocateArray(C_DOUBLE, -1., -0.15, -1., -0.02, -0.198997487421324, -1., -0.1,
                    -0.16583123951776996, -0.158113883008419, -1., -0.15, -0.10552897060221729, -0.17392527130926083,
                    -0.16159714218895202, 1., -1.);
            val gjcSeg = arena.allocateArray(C_LONG_LONG, 0, 2, 5, 9, 14, 16);
            val girSeg = arena.allocateArray(C_LONG_LONG, 0, 6, 1, 6, 7, 2, 6, 7, 8, 3, 6, 7, 8, 9, 4, 5);
            val aprSeg = arena.allocateArray(C_DOUBLE, 1., 1., 1., 1.);
            val ajcSeg = arena.allocateArray(C_LONG_LONG, 0, 1, 2, 3, 4, 4);
            val airSeg = arena.allocateArray(C_LONG_LONG, 0, 0, 0, 0);
            val cSeg = arena.allocateArray(C_DOUBLE, -0.05, -0.06, -0.08, -0.06, 0.);
            val hSeg = arena.allocateArray(C_DOUBLE, 0., 0., 0., 0., 0.2, 0., 0., 0., 0., 0.);
            val bSeg = arena.allocateArray(C_DOUBLE, 1.);
            ...
            val stgsSeg = pwork.stgs$get(workSeg).reinterpret(settings.sizeof(), arena, null);
            settings.verbose$set(stgsSeg, 0);
